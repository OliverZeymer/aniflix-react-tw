...
const Anime = () => {
  ...
  let API_URL = `https://api.jikan.moe/v4/anime?order_by=${order || "members" || (order === "null" && "members")}&sort=desc&limit=${limit}&status=${
  ...
  return (
    <main>
      ...
      <SelectFilters status={status} page={page} order={order} search={search} />

      <PageNavigation status={status} page={page} order={order} search={search} mb="0" mt="8" />
      <Toggle />
      <section className="sm:grid sm:grid-cols-auto-fit flex flex-col gap-6 w-full h-fit">
        {error && <p>{error}</p>}
        {isLoading ? <Loader /> : <AnimeCard data={data} />}
      </section>
      <PageNavigation status={status} page={page} order={order} search={search} mt="8" bottom="true" />
    </main>
  );
};
